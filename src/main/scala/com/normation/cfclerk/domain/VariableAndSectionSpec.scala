...
object DisplayPriorityFactory {
  def apply(s: String) : DisplayPriority = {
    s.toLowerCase match {
      case HighDisplayPriority.priority => HighDisplayPriority
      case LowDisplayPriority.priority  => LowDisplayPriority
      case _                            => HighDisplayPriority
    }
  }
}